...
function build_wheel() {
  ...
  sip-wheel --verbose --qmake /usr/local/opt/qt/bin/qmake --build-dir builddir  --build-tag cp39 --no-manylinux --debug --pep484-pyi --tracing  --name PyQt5 --qml-debug --target-qt-dir /workspaces/qt-build/build --confirm-license -qt-shared --no-designer-plugin --no-docstrings   
  mkdir -p ${DEST}
  cp dist/* ${DEST}
  popd > /dev/null
